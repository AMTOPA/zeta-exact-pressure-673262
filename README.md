...
> **Research status.** This is a same-day research draft, not a peer-reviewed
> theorem and not a proof of the Riemann hypothesis. The new contribution here
> is a finite combinatorial averaging identity. The analytic interface and the
> seven-point interval certificate are imported from prior work and are not
> reverified in this repository.
...
later costs \(B(m-1)\) over the shifted block partitions. If the exact
\(c_j\)'s are retained until the shifted-partition average, each global gap
occupies every internal block position exactly once, so its total charge over
all \(m\) shifts is instead
...
\[
H_{\rm cert}=\frac{672457041414544284}{10^{18}},\qquad
...
\[
A_m=\varepsilon(m-6),\qquad R_m=h_m(A_m),\qquad
\eta_m=\frac{R_m}{A_m},
\]
...
\ge
\frac{mH_{\rm cert}-\eta_m B(m-6)}{m-R_m}-o(1).
\]
...
`check_final_bound.py` performs the high-precision final arithmetic and scans
the block length.
...
- the final high-precision arithmetic;
- the integer scan selecting \(m=215\).

### Imported

- the analytic inequality \(S\ge H_{\rm cert}N+\operatorname{tr}\Psi(M)-o(N)\);
- the certified local seven-point inequality \(F\ge 51063/10^7\);
- the sharp finite-\(m\) Gram profile \(h_m\);
- the normalization that the total normalized simple-zero gap length is \(N+o(N)\).
...
refinements are due to the repositories listed in `docs/lineage.md`.
